...
using CSV, DataFrames, ScikitLearn, Statistics, CairoMakie, ColorSchemes, ManifoldLearning
...
begin
	download("https://archive.ics.uci.edu/ml/machine-learning-databases/wine/wine.data", "wine_data")
	data = CSV.read("wine_data", header=header, DataFrame)
end
...
X = Matrix(select(data, Not("Variety")))
...
# ╔═╡ 2ae4446b-5fbc-4b4b-b25f-9ad5058aa6c2
transformed_pca = pca.fit_transform(X)

# ╔═╡ 2d0e97f1-270e-4f16-a198-67d40ca5a4fa
transformed_pca[:, 1]

# ╔═╡ c59f56e3-bca6-4bff-8c9c-1e92f8870b44
...
begin
	local fig = Figure()
	local ax=Axis(fig[1,1], xlabel="PC 1", ylabel="PC 2")

	variety_1 = transformed_pca[data[:, "Variety"].==1, :]	
	variety_2 = transformed_pca[data[:, "Variety"].==2, :]
	variety_3 = transformed_pca[data[:, "Variety"].==3, :]
	scatter!(variety_1[:, 1], variety_1[:, 2], color =colors[1], label="variety = 1")
	scatter!(variety_2[:, 1], variety_2[:, 2], color =colors[2], label="variety = 2")
	scatter!(variety_3[:, 1], variety_3[:, 2], color =colors[3], label="variety = 3")
...
# ╔═╡ 76b295df-6ef0-42db-9127-75fb790b8412
variance = pca.explained_variance_ratio_

# ╔═╡ 9f4ca36d-3c2c-46a6-9f3a-e161f104ab7d
variance[1] + variance[2]

# ╔═╡ 22ff009f-ab22-48f9-aea8-9eda6465a7f9
md"""
**Diffusion Map**
"""

# ╔═╡ 7ad5882b-b44e-46d0-92b0-8d2e5f6368a9
X′= transpose(X)

# ╔═╡ 98a78b49-f7e2-4c0c-a0af-3eb5ed39ce77
begin
	kernel = (x, y) -> x' * y # linear kernel
	map = fit(DiffMap, X′, kernel=kernel)
	 
end

# ╔═╡ f6082d42-0c9c-4d75-b42b-916c8f32865e
transformation = transpose(ManifoldLearning.transform(map))

# ╔═╡ e000f032-8dd6-4263-bdf3-85f88a46e5fd
begin
	local fig = Figure()
	local ax=Axis(fig[1,1], xlabel="PC 1", ylabel="PC 2")

	diff_variety_1 = transformation[data[:, "Variety"].==1, :]	
	diff_variety_2 = transformation[data[:, "Variety"].==2, :]
	diff_variety_3 = transformation[data[:, "Variety"].==3, :]
	scatter!(diff_variety_1[:, 1], diff_variety_1[:, 2], color =colors[1], label="variety = 1")
	scatter!(diff_variety_2[:, 1], diff_variety_2[:, 2], color =colors[2], label="variety = 2")
	scatter!(diff_variety_3[:, 1], diff_variety_3[:, 2], color =colors[3], label="variety = 3")
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "8d06040f88dadb3e2e0853ce7c8b2dfebd0d9e18"
